...
- For daemon changes, preserve the rule that background jobs must not edit tracked source files in the checked-out working tree or apply agent changes there. There are two repo metadata exceptions. First, snapshot ignore setup: `.gitignore` may be updated by `roborev init`, and daemon snapshot writes may add a local `.git/info/exclude` fallback. Ignored repo-local snapshot artifacts are allowed only for oversized prompt or diff handoff and must stay under the configured `snapshot_dir`, remain gitignored, and be cleaned up best-effort. Second, remote callers: pack uploads may write git objects and `refs/roborev/uploads/<sha>` refs into a registered clone, and remote enqueues may run `git fetch` there, as long as neither touches the working tree, the index, or branches.
