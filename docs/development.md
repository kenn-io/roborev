...
The daemon's typed HTTP API uses Go's JSON v2 encoder. Ordinary slice fields are
non-nullable collections in OpenAPI and serialize as `[]` when empty. Use an
explicit nullable representation only when `null` is part of the API contract.
