...
func startStreamHandler(t *testing.T, server *Server, url string) (context.CancelFunc, *safeRecorder, chan struct{}) {
	...
	require.True(t, waitForSubscriberIncrease(server.broadcaster, initialCount), "Timed out waiting for subscriber")
...
func TestHandleStreamEvents(t *testing.T) {
	...
	t.Run("streams events as JSONL", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			...
			require.True(t, waitForEvents(w, 2), "Timed out waiting for events")
	...
	t.Run("all event types are supported", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			...
			require.True(t, waitForEvents(w, 4), "Timed out waiting for events")
			cancel()
			<-done
	...
	t.Run("omitempty fields are excluded when empty", func(t *testing.T) {
		synctest.Test(t, func(t *testing.T) {
			...
			require.True(t, waitForEvents(w, 1), "Timed out waiting for events")
			cancel()
			<-done
