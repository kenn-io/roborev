...
func assertContainsAll(t *testing.T, s, wantLabel string, subs ...string) {
	t.Helper()
...
func TestCIPollerFindOrCloneRepo_InvalidExistingDir(t *testing.T) {
	tests := []struct {
		name     string
		repoName string
		setupFs  func(t *testing.T, dataDir, clonePath string)
	}{
		{
			name:     "empty dir is re-cloned",
			repoName: "acme/empty",
			setupFs: func(t *testing.T, _, p string) {
		...
		},
		{
			...
			setupFs: func(t *testing.T, dataDir, p string) {
