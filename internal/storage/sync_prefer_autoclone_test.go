...
func TestPreferAutoCloneNormalizesPathSeparators(t *testing.T) {
	...
	t.Run("matches forward slash stored clone path", func(t *testing.T) {
		clone := Repo{ID: 1, RootPath: filepath.ToSlash(filepath.Join(dataDir, "clones", "repo")), Identity: "remote", CreatedAt: time.Unix(1, 0)}
		recent := Repo{ID: 2, RootPath: filepath.ToSlash(filepath.Join(dataDir, "checkout")), Identity: "remote", CreatedAt: time.Unix(2, 0)}
	...
	t.Run("requires the clone boundary slash", func(t *testing.T) {
		clonePrefixLookalike := Repo{ID: 1, RootPath: filepath.ToSlash(filepath.Join(dataDir, "clones-other", "repo")), Identity: "remote", CreatedAt: time.Unix(1, 0)}
		recent := Repo{ID: 2, RootPath: filepath.ToSlash(filepath.Join(dataDir, "checkout")), Identity: "remote", CreatedAt: time.Unix(2, 0)}
